...
for e in range(61):#per electrode
  ...
  #change the shape of less stressed group from (25,6,60) to (25*6,60)
  non_x = []
  ...
  #change the shape of more stressed group from (25,6,60) to (25*6,60)
  yes_x = []
  ...
  #change the shape of concatenated group from (50,6,60) to (50*6,60)
  x = []
  ...
  reshaped_non = np.empty((25, 6, 1, 60))

  for i in range(25):
      for j in range(6):
          reshaped_non[i, j, :, :] = expand_non[i * 6 + j, :, :]

  # create an empty array
  reshaped_yes = np.empty((25, 6, 1, 60))

  for i in range(25):
      for j in range(6):
          reshaped_yes[i, j, :, :] = expand_yes[i * 6 + j, :, :]

  # create an empty array
  reshaped_all = np.empty((50, 6, 1, 60))

  for i in range(50):
      for j in range(6):
          reshaped_all[i, j, :, :] = expand_all[i * 6 + j, :, :]
  ...
  for k, (train_index, test_index) in enumerate(kf.split(reshaped_all)):
    ...
    #change the shape of train data from (49,6,1,60) to (49*6,1,60)
    X_train = []
    Y_train = []
    ...
    #change the shape of val data from (1,6,1,60) to (1*6,1,60)
    X_val = []
    Y_val = []
