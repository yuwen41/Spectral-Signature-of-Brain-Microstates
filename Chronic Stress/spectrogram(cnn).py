...
non_stress = pd.read_csv('./Y_label(csv)/less_stress.csv')#change your directory
non_subject = non_stress.iloc[:,0].tolist()
stress = pd.read_csv('./Y_label(csv)/more_stress.csv')#change your directory
...
for e in range(61):#per electrode
  ...
  reshaped_non = np.empty((25, 6, 60, 119))
  ...
  # create an empty array
  reshaped_yes = np.empty((25, 6, 60, 119))
  ...
  # create an empty array
  reshaped_all = np.empty((50, 6, 60, 119))
  ...
  for k, (train_index, test_index) in enumerate(kf.split(reshaped_all)):
    ...
    #change the shape of train data from (49,6,60,119) to (49*6,60,119)
    X_train = []
    Y_train = []
    ...
    #change the shape of train data from (1,6,60,119) to (1*6,60,119)
    X_val = []
    Y_val = []
...
df.to_csv('./.../spectrogram(cnn).csv', index=False)#change your directory
