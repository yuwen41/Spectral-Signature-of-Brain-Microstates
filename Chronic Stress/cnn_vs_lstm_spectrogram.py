...
#Chronic Stress: CNN vs LSTM
cnn_stress = pd.read_csv('./results and electrode names(csv)/spectrogram(cnn).csv')#change your directory
cnn_accuracy = cnn_stress.iloc[50,:].tolist()#LOOCV accuracy
cnn_se = cnn_stress.iloc[52,:].tolist()#standard error

lstm_stress = pd.read_csv('./results and electrode names(csv)/spectrogram(lstm).csv')#change your directory
lstm_accuracy = lstm_stress.iloc[50,:].tolist()#LOOCV accuracy
lstm_se = lstm_stress.iloc[52,:].tolist()#standard error

electrode = pd.read_csv('./results and electrode names(csv)/electrode_names.csv')#change your directory
electrode = electrode['Channel Names'].tolist()#get electrode name
...
plt.show()
plt.savefig('./figure/CNN_vs_LSTM_spectrogram.svg', bbox_inches='tight')#your directory